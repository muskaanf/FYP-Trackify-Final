...
const generateRandomData = async (req, res) => {
    const videoRefs = ["North Nazimabad", "Defence Housing Society", "Bahadurabad PECHS", "Malir Cannt"]; // Replace with actual video references
    const timestamp = new Date();

    try {
        const randomData = videoRefs.map(videoRef => ({
            number: Math.floor(Math.random() * 101) + 200, // Random number between 200 and 300
            video_ref: videoRef,
            time: timestamp
        }));
